...
import { ArrowLeft, Clock, CheckCircle, MapPin, CreditCard, Share2, Image as ImageIcon, Car, AlertTriangle, Timer } from 'lucide-react';
...
type Step = 'timeslot' | 'payment' | 'confirmation';

interface LocationState {
  vehicle?: { type: string; brand?: string; model?: string; plateNumber?: string };
  location?: string;
}

// Generate available 1-hour time slots
...
export function UnifiedBooking() {
  ...
  const [activeVehicle] = useState({
    model: state?.vehicle ? `${state.vehicle.brand} ${state.vehicle.model}` : 'No Vehicle Selected',
  ...
  const getSlotStart = () => bookingData.selectedSlot.split(' - ')[0] || '';
  const getSlotEnd = () => bookingData.selectedSlot.split(' - ')[1] || '';
  ...
  const handleShare = async () => {
    const shareText = `PakiPark Reservation\nLocation: ${bookingData.location}\nVehicle: ${activeVehicle.plate}\nTime Slot: ${bookingData.selectedSlot}\nDate: ${bookingData.date}`;
    if (navigator.share) {
      try {
        await navigator.share({ title: 'PakiPark E-Pass', text: shareText, url: window.location.href });
    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col font-sans text-gray-800 print:bg-white relative">
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-50 px-8 py-4 flex justify-between items-center w-full print:hidden">
        ...
        <nav className="hidden lg:flex items-center gap-10">
          {['Time Slot', 'Payment', 'Confirm'].map((step, i) => {
            const stepKey = ['timeslot', 'payment', 'confirmation'][i];
      ...
      <main className="flex-1 w-full max-w-[1400px] mx-auto p-6 lg:p-8">
        ...
        {currentStep === 'timeslot' && (
          <div className="w-full max-w-7xl mx-auto px-6 lg:px-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-start min-h-[70vh]">
              <div className="lg:col-span-5 space-y-8 pt-8">
                ...
                <p className="text-lg text-gray-500 max-w-md leading-relaxed">
                  Pick a date and a 1-hour window. Arrive anytime within your slot at {bookingData.location}.
                </p>

                {/* How it works */}
                <div className="space-y-3">
                  <h4 className="text-xs font-bold text-[#1e3d5a] uppercase tracking-widest">How it works</h4>
                  {[
                    { icon: <Clock className="size-4" />, text: "Reserve a specific 1-hour time slot" },
                    { icon: <Car className="size-4" />, text: "Arrive at any point within that hour" },
                    { icon: <AlertTriangle className="size-4" />, text: "No-show? Reservation is automatically voided" },
                    { icon: <Timer className="size-4" />, text: "Stay longer? Pay based on extra time used" },
                  ].map((item, i) => (
                    <div key={i} className="flex items-center gap-3 bg-white px-4 py-3 rounded-xl border border-gray-100 shadow-sm">
                      <div className="text-[#ee6b20]">{item.icon}</div>
              ...
              <div className="lg:col-span-7">
                <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-gray-200/50 border border-gray-50 p-8 lg:p-10 relative overflow-hidden">
                  <form onSubmit={(e) => {
                    e.preventDefault();
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-[320px] overflow-y-auto pr-1 custom-scrollbar">
                        {timeSlots.map((slot) => {
                          ...
                          return (
                            ...
                            >
                              <Clock className={`size-4 mx-auto mb-1 ${isSelected ? 'text-[#ee6b20]' : 'text-gray-400'}`} />
                              {slot}
                            </button>
                          );
                        })}
                    ...
                    <Button
                      ...
                      className="w-full bg-[#ee6b20] hover:bg-[#d95a10] h-14 rounded-xl font-bold uppercase tracking-widest shadow-lg transition-all hover:translate-y-[-2px] disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      Next: Payment
                    </Button>
                  </form>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start max-w-6xl mx-auto animate-in fade-in duration-500">
            <div className="lg:col-span-4 bg-[#1e3d5a] rounded-3xl p-8 text-white space-y-6 shadow-xl">
              ...
              <div className="space-y-3 opacity-80 text-sm">
                ...
                <div className="flex justify-between"><span>Time Slot</span><span className="font-bold">{bookingData.selectedSlot}</span></div>
                <div className="flex justify-between"><span>Duration</span><span>1 hour</span></div>
                <div className="flex justify-between"><span>Rate</span><span>₱{HOURLY_RATE}.00/hr</span></div>
              ...
              <div className="bg-white/10 rounded-xl p-3 text-xs leading-relaxed">
                <p className="opacity-80">
                  <strong className="text-[#ee6b20]">Note:</strong> If you stay beyond your reserved hour, additional charges of ₱{HOURLY_RATE}/hr will apply based on actual time used.
                </p>
              </div>
            </div>
            <div className="lg:col-span-8 bg-white rounded-3xl border border-gray-100 p-10 shadow-sm">
              ...
              <div className="flex gap-4">
                <Button variant="outline" onClick={() => setCurrentStep('timeslot')} className="h-14 px-8 rounded-xl font-bold">Back</Button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
              <div className="lg:col-span-5 space-y-8 print:hidden">
                <div>
                  ...
                  <p className="text-lg text-gray-500 mt-4 leading-relaxed">Review your E-Ticket details for {activeVehicle.model}.</p>
                </div>
                ...
                <div className="bg-orange-50 border border-orange-100 p-6 rounded-[2rem] flex items-start gap-4 shadow-sm">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-gray-500 text-sm leading-relaxed">
                      Show the <span className="font-bold text-[#ee6b20]">barcode</span> on your E-Pass to the parking attendant. You may arrive anytime within your reserved hour.
                    </p>
                  </div>
                </div>

                <div className="bg-amber-50 border border-amber-100 p-6 rounded-[2rem] flex items-start gap-4 shadow-sm">
                  <div className="size-10 bg-amber-500 rounded-full flex items-center justify-center shrink-0 shadow-lg shadow-amber-200">
                    <AlertTriangle className="size-5 text-white" />
                  </div>
                  <div className="space-y-1">
                    <h4 className="font-bold text-[#1e3d5a] text-sm uppercase tracking-wider">No-Show Policy</h4>
                    <p className="text-gray-500 text-sm leading-relaxed">
                      If you fail to arrive within your time slot, the reservation will be <span className="font-bold text-amber-600">automatically voided</span>. Penalty rules may apply.
                    </p>
                  </div>
                </div>

                <Button variant="ghost" onClick={() => setCurrentStep('payment')} className="w-full h-14 text-gray-400 hover:text-[#1e3d5a] font-bold">
              ...
              <div className="lg:col-span-7 bg-white rounded-[3rem] border border-gray-200 shadow-2xl overflow-hidden relative max-w-[460px] mx-auto print:shadow-none print:border-none">
                <div className="bg-[#1e3d5a] p-8 text-white relative">
                  ...
                  <h3 className="text-2xl font-bold text-[#ee6b20]">1-Hour Reservation</h3>
                </div>
                <div className="p-8 space-y-6 relative">
                  ...
                  <div className="grid grid-cols-2 gap-y-6 gap-x-4 border-b border-gray-100 pb-6 text-sm">
                    ...
                    <div className="space-y-0.5">
                      <div className="text-[9px] font-bold text-gray-400 uppercase">Vehicle</div>
                  ...
                  <div className="py-2 flex flex-col items-center gap-2">
                    <div className="w-full h-14 bg-white flex items-center justify-center px-4 overflow-hidden">
                      {barcodeWidths.map((bar, i) => (
                        <div key={i} className="bg-black shrink-0" style={{ width: `${bar.width}px`, height: '100%', marginLeft: '1px', opacity: bar.visible ? 1 : 0 }} />
                      ))}
                    </div>
                    <p className="text-[9px] font-mono text-gray-400 tracking-[0.6em] uppercase">PKP-{activeVehicle.plate.replace(/\s/g, '')}</p>
                  </div>
                  ...
                  <Button onClick={async () => {
                    ...
                    try {
                      const result = await bookingService.createBooking({
                        vehicleId: (state?.vehicle as any)?._id || '',
                        locationId: '',
                        spot: 'Auto-Assigned',
                        date: bookingData.date,
                      ...
                      setBookingReference(result.reference || result._id || '');
                      toast.success('Booking confirmed!');
                    } catch {
                      // Fallback: proceed anyway for demo purposes
                      setBookingReference(`PKP-${Date.now().toString(36).toUpperCase()}`);
                    }
                    setIsConfirming(false);
                    setShowSuccessModal(true);
                  }} disabled={isConfirming} className="w-full bg-[#1e3d5a] hover:bg-[#152b40] h-16 rounded-2xl text-white font-bold text-lg uppercase tracking-[0.1em] shadow-xl print:hidden">
                    {isConfirming ? 'Processing...' : 'Confirm Reservation'}
                  </Button>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white w-full max-w-xs aspect-square flex flex-col items-center justify-center p-10 rounded-[2rem] border border-gray-100 shadow-2xl text-center">
            ...
            <div className="space-y-2 mb-8">
              ...
              <p className="text-gray-500 text-xs leading-relaxed px-2">
                ...
                <br />Slot: {bookingData.selectedSlot}
              </p>
            </div>
            <Button
