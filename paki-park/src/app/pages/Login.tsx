...
export function Login() {
  ...
  const [loginMethod, setLoginMethod] = useState<"customer" | "admin">("customer");
  ...
  useEffect(() => {
    if (loginMethod === "customer" && /[a-zA-Z@]/.test(formData.identifier)) {
      setIsEmail(true);
  ...
  }, [formData.identifier, loginMethod]);

  const handleIdentifierChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    if (loginMethod === "customer") {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (loginMethod === "admin") {
      if (!emailRegex.test(formData.identifier)) {
        newErrors.identifier = "Business Partner login requires a valid email format.";
        hasError = true;
      }
    }
    ...
    setIsLoading(true);
    try {
      // Determine email — if customer used phone, construct email-like identifier
      const email = isEmail ? formData.identifier : `+63${formData.identifier}`;
      ...
      // Fallback: if backend is unreachable, use localStorage approach
          ...
          <div className="w-full max-w-[520px] bg-white rounded-[40px] shadow-[0_20px_60px_-15px_rgba(30,61,90,0.15)] p-8 sm:p-14 border border-white relative">
            ...
            <div className="mb-10 text-left">
              ...
              <p className="text-[#8492a6] font-medium text-sm">
                {loginMethod === "customer" ? "Welcome back! Login via email or phone." : "Access the Business Partner panel."}
              </p>
            </div>

            <div className="flex gap-1 mb-8 bg-[#f0f4f8] p-1.5 rounded-2xl border border-[#e2e8f0]">
              {(["customer", "admin"] as const).map((method) => (
                <button
                  key={method}
                  type="button"
                  onClick={() => { setLoginMethod(method); setFormData({ identifier: "", password: "" }); setErrors({ identifier: "", password: "" }); }}
                  className={`flex-1 py-3 px-1 rounded-xl text-xs sm:text-sm font-bold capitalize transition-all duration-300 flex items-center justify-center gap-2 ${
                    loginMethod === method ? "bg-[#1e3d5a] text-white shadow-lg" : "text-[#8492a6] hover:text-[#1e3d5a]"
                  }`}
                >
                  {method === "customer" ? <User className="size-4" /> : <UserCog className="size-4" />}
                  {method === "customer" ? "Customer" : "Business Partner"}
                </button>
              ))}
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-1.5 text-left">
                <Label className="text-[10px] font-bold text-[#1e3d5a] tracking-widest uppercase opacity-70 px-1">
                  {loginMethod === "customer" ? "Email or Mobile Number" : "Business Partner Email"}
                </Label>
                <div className="flex gap-2">
                  {!isEmail && loginMethod === "customer" && (
                  ...
                  <div className="flex-1 relative">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#8492a6]">
                      {isEmail ? <Mail className="size-5" /> : <Phone className="size-5" />}
                    </div>
                    <Input
                      type="text"
            ...
            {!resetSent ? (
              <div className="space-y-6">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[#8492a6] text-sm font-medium mt-1">Enter your details to receive a link.</p>
                </div>
                <Input type="text" value={forgotIdentifier} onChange={(e) => setForgotIdentifier(e.target.value)} className="h-14 bg-[#f8fafc] border border-[#e2e8f0] rounded-xl px-4" />
