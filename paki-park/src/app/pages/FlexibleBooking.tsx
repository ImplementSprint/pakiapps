...
import { ArrowLeft, Clock, CheckCircle, MapPin, Ticket, Printer, ShieldCheck, CreditCard, Share2, Image as ImageIcon, Car, Zap } from 'lucide-react';
...
export function FlexibleBooking() {
  ...
  const [bookingData, setBookingData] = useState({
    ...
    vehicleType: state?.vehicle?.type || 'Sedan', // Included vehicle type
    vehicleModel: state?.vehicle ? `${state.vehicle.brand} ${state.vehicle.model}` : 'Active Vehicle',
  ...
  const handleShare = async () => {
    const shareText = `PakiPark Flexible Session\nLocation: ${bookingData.location}\nVehicle: ${bookingData.plateNumber}\nArrival: ${bookingData.date} @ ${bookingData.startTime}`;
    if (navigator.share) {
      try {
    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col font-sans text-gray-800 print:bg-white relative">
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-50 px-8 py-4 flex justify-between items-center w-full print:hidden">
        ...
        <nav className="hidden lg:flex items-center gap-10">
          {['Arrival', 'Payment', 'Confirm'].map((step, i) => {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center min-h-[70vh]">
              <div className="lg:col-span-7 space-y-8">
                <h2 className="text-5xl lg:text-6xl font-bold text-[#1e3d5a] leading-tight">
                  The <span className="text-[#ee6b20]">Flexible</span> way <br/>to park.
                </h2>
                <div className="flex items-center gap-3 bg-white w-fit px-5 py-3 rounded-2xl border border-gray-100 shadow-sm">
                  ...
                  <span className="text-sm font-bold text-[#1e3d5a]">
                    Booking for: {bookingData.plateNumber} ({bookingData.vehicleModel})
                  </span>
                </div>
                <p className="text-lg text-gray-500 max-w-md leading-relaxed">
                  Reserve your arrival time and stay as long as you need. Pay only for the time you actually use at {bookingData.location}.
                </p>
              </div>
              ...
              <div className="lg:col-span-5">
                <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-gray-200/50 border border-gray-50 p-10 lg:p-12 relative overflow-hidden">
                  <form onSubmit={(e) => { e.preventDefault(); setCurrentStep('payment'); }} className="space-y-6 relative z-10">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Arrival Time</label>
        ...
        {currentStep === 'payment' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start max-w-6xl mx-auto animate-in fade-in duration-500">
            <div className="lg:col-span-4 bg-[#1e3d5a] rounded-3xl p-8 text-white space-y-6 shadow-xl">
               <h3 className="text-xl font-bold">Reservation Fee</h3>
               <div className="space-y-3 opacity-80 text-sm">
                 <div className="flex justify-between"><span>Base Deposit</span><span>₱50.00</span></div>
                 ...
                 <div className="flex justify-between"><span>Hourly Rate</span><span>₱50.00/hr</span></div>
               </div>
               <div className="pt-6 border-t border-white/10 flex justify-between items-end">
                 <span className="font-bold text-[10px] uppercase opacity-60">To Pay Now</span>
                 <span className="text-4xl font-bold text-[#ee6b20]">₱{RESERVATION_FEE}</span>
               </div>
            </div>
            <div className="lg:col-span-8 bg-white rounded-3xl border border-gray-100 p-10 shadow-sm">
              ...
              <div className="flex gap-4">
                ...
                <Button onClick={handleProceedToConfirm} className="flex-1 bg-[#1e3d5a] h-14 rounded-xl text-white font-bold uppercase tracking-widest shadow-lg">Review Reservation</Button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="max-w-5xl mx-auto animate-in fade-in zoom-in-95 duration-500">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
              <div className="lg:col-span-5 space-y-8 print:hidden">
                <div>
                  <h2 className="text-4xl font-bold text-[#1e3d5a] leading-tight">Confirm Your <br/><span className="text-[#ee6b20]">Arrival Pass</span></h2>
                  <p className="text-lg text-gray-500 mt-4 leading-relaxed">Review your flexible session details for {bookingData.location}.</p>
                </div>
              ...
              <div className="lg:col-span-7 bg-white rounded-[3rem] border border-gray-200 shadow-2xl overflow-hidden relative max-w-[460px] mx-auto print:shadow-none print:border-none">
                <div className="bg-[#1e3d5a] p-8 text-white relative">
                   ...
                   <h3 className="text-2xl font-bold text-[#ee6b20]">Flexible Session</h3>
                </div>
                <div className="p-8 space-y-6 relative">
                  ...
                  <div className="grid grid-cols-2 gap-y-6 gap-x-4 border-b border-gray-100 pb-6 text-sm">
                    ...
                    <div className="space-y-0.5">
                      <div className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Customer</div>
                      <p className="font-bold text-[#1e3d5a] truncate">{userData.name}</p>
                    </div>
                    <div className="space-y-0.5 text-right">
                      <div className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Plate Number</div>
                      <p className="font-bold text-[#ee6b20]">{bookingData.plateNumber}</p>
                    </div>
                    <div className="space-y-0.5">
                      <div className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Vehicle Type</div>
                      <p className="font-bold text-[#1e3d5a]">{bookingData.vehicleType}</p>
                    </div>
                    <div className="space-y-0.5 text-right">
                      <div className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Target Arrival</div>
                      <p className="font-bold text-[#1e3d5a]">{bookingData.startTime}</p>
                    </div>
                  </div>
                  ...
                  <div className="py-2 flex flex-col items-center gap-2">
                    ...
                    <p className="text-[9px] font-mono text-gray-400 tracking-[0.6em] uppercase">PKP-FLEX-{bookingData.plateNumber.replace(/\s/g, '')}</p>
                  </div>
          ...
          <div className="bg-white w-full max-w-xs aspect-square flex flex-col items-center justify-center p-10 rounded-[2rem] border border-gray-100 shadow-2xl text-center">
            ...
            <div className="space-y-2 mb-8">
              ...
              <p className="text-gray-500 text-xs leading-relaxed px-2">
                Arrival for <span className="font-bold text-gray-900">{bookingData.plateNumber}</span> is set. <br /> See you at {bookingData.location}!
              </p>
            </div>
            <Button onClick={() => navigate('/customer/home')} className="w-full h-12 bg-[#ee6b20] hover:bg-[#d95a10] text-white font-bold rounded-xl shadow-md transition-all active:scale-95">Go to Dashboard</Button>
          </div>
        </div>
      )}
    </div>
